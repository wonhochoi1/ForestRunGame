...
void drawImageDMA(int row, int col, int width, int height, const u16 *image) {
  ...
  for (int i = 0; i < height; i++) {
    ...
    DMA[DMA_CHANNEL_3].dst = videoBuffer + ((row + i) * WIDTH) + col;
...
void undrawImageDMA(int row, int col, int width, int height, const u16 *image) {
  ...
  for (int i = 0; i < height; i++) {
    DMA[DMA_CHANNEL_3].src = image + OFFSET(i, col, width);
    DMA[DMA_CHANNEL_3].dst = videoBuffer + (row * WIDTH) + col;
    DMA[DMA_CHANNEL_3].cnt = width | DMA_ON;
  }
}
...
void fillScreenDMA(volatile u16 color) {
  ...
  DMA[DMA_CHANNEL_3].src = &color;
  DMA[DMA_CHANNEL_3].dst = videoBuffer;
  DMA[DMA_CHANNEL_3].cnt = (WIDTH * HEIGHT) | DMA_SOURCE_FIXED | DMA_ON;
}
